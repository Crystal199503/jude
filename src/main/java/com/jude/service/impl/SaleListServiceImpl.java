...
public class SaleListServiceImpl implements SaleListService {
	...
	public void save(SaleList saleList, List<SaleListGoods> saleListGoodsList) {
		...
		for(SaleListGoods saleListGoods:saleListGoodsList){
			saleListGoods.setType(goodsTypeRepository.findById(saleListGoods.getTypeId()).get()); // 设置类别
	...
	public List<SaleListGoods> listBySaleName(String name, String custom_id, String bSaleDate, String eSaleDate) {
		return saleListGoodsRepository.listBySaleName(name,custom_id,bSaleDate,eSaleDate);
	}
